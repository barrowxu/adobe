...
U32 FCXForm::MinBits(){
	...
	return FSWFStream::MinBits(maxValue, 1);
}
...
void FCXForm::WriteToSWFStream(FSWFStream* _SWFStream){
	...
	if (hasMult){
		_SWFStream->WriteBits((U32)redMultTerm, nBits);
		_SWFStream->WriteBits((U32)greenMultTerm, nBits);
		_SWFStream->WriteBits((U32)blueMultTerm, nBits);
	}
	if (hasAdd){
		_SWFStream->WriteBits((U32)redAddTerm, nBits);
		_SWFStream->WriteBits((U32)greenAddTerm, nBits);
		_SWFStream->WriteBits((U32)blueAddTerm, nBits);
	}
...
void FCXFormWAlpha::WriteToSWFStream(FSWFStream* _SWFStream){
	...
	if (hasMult){
		...
		_SWFStream->WriteBits((U32)greenMultTerm, nBits);
		_SWFStream->WriteBits((U32)blueMultTerm, nBits);
		_SWFStream->WriteBits((U32)alphaMultTerm, nBits);
	}
	if (hasAdd){
		_SWFStream->WriteBits((U32)redAddTerm, nBits);
		_SWFStream->WriteBits((U32)greenAddTerm, nBits);
		_SWFStream->WriteBits((U32)blueAddTerm, nBits);
		_SWFStream->WriteBits((U32)alphaAddTerm, nBits);
	}
...
{    
	...
	{
		delete fillStyleArray.front();
		fillStyleArray.pop_front();
	}
}
...
void FFillStyleArray::WriteToSWFStream(FSWFStream *_SWFStream){
	...
	std::list<FAFillStyle*>::iterator cursor;
...
void FShape::WriteToSWFStream(FSWFStream *_SWFStream){
	...
	std::list<FShapeRec*>::iterator cursor;
