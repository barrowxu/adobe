...
private:
	...
	std::list<FAFillStyle*> fillStyleArray;
...
/*!	Note that ostensibly the ratio parameter is the percentage complete along 
...
Normally the Colors in a gradient array store the colors along the gradient.  These
store pairs of colors, one for each of the two objects along with the mysterious ratio number.
...
private:
	...
	U32 nFillBits;
	U32 nLineBits;
	std::list<FShapeRec*> shapeRecs;       //see comments, will need to be changed
